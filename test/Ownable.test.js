...
contract('BaseVestingTest', ([owner, user1, user2, user3, user4]) => {
    ...
    describe('Tests', async () => {
        ...
        it('Check constructor', async () => {
            await expectRevert(BaseVesting.new(constants.ZERO_ADDRESS, owner, startDate, {
                from: owner
            }), "Invalid reward token address");
            await expectRevert(BaseVesting.new(rewardToken.address, constants.ZERO_ADDRESS, startDate, {
                from: owner
            }), "Invalid signer address");
            await expectRevert(BaseVesting.new(rewardToken.address, owner, 0, {
                from: owner
        ...
        describe('Time testing', async () => {
            it('Before contract started', async () => {
                await expectRevert(baseVesting.getRewardBalance(ether('100'), {
                    from: user3
                }), "You don't have permissions");
                expect(await baseVesting.getRewardBalance(ether('100'), {
                    from: owner
            ...
            it('StartDate <= now < FirstRelease', async () => {
                ...
                expect(await baseVesting.getRewardBalance(ether('50'), {
                    from: user1
                })).to.be.bignumber.equal(ether('250'));
                expect(await baseVesting.getRewardBalance(ether('100'), {
                    from: user1
                })).to.be.bignumber.equal(ether('500'));
                expect(await baseVesting.getRewardBalance(ether('50'), {
                    from: user2
                })).to.be.bignumber.equal(ether('750'));
                expect(await baseVesting.getRewardBalance(ether('100'), {
                    from: user2
                })).to.be.bignumber.equal(ether('1500'));
                //Check withDrawTokens
                expect(await rewardToken.balanceOf(user1)).to.be.bignumber.equal(ether('0'));
                await baseVesting.withdrawTokens(ether('50'), {
                    from: user1
                });
                expect(await rewardToken.balanceOf(user1)).to.be.bignumber.equal(ether('250'));
                await baseVesting.withdrawTokens(ether('100'), {
                    from: user2
                });
                expect(await rewardToken.balanceOf(user2)).to.be.bignumber.equal(ether('1500'));
                await expectRevert(baseVesting.withdrawTokens(ether('100'), {
                    from: user2
                }), "No rewards available");
                await expectRevert(baseVesting.withdrawTokens(ether('100'), {
                    from: user3
                }), "You don't have permissions");
                await expectRevert(baseVesting.withdrawTokens(ether('1000'), {
                    from: user2
                }), "Can't get more than 100% of tokens");
                //Check setInitialTimestamp
                let currentDate = await time.latest();
                await expectRevert(baseVesting.setInitialTimestamp(
                    currentDate.add(time.duration.minutes(20))), 
                    "Contract already start staking"
                );
            })
            it('FirstRelease <= now < VestingTimeEnd', async () => {
                ...
                expect(await baseVesting.getRewardBalance(ether('100'), {
                    from: user1
                })).to.be.bignumber.equal(ether('2750')); //55%
                expect(await baseVesting.getRewardBalance(ether('50'), {
                    from: user1
                })).to.be.bignumber.equal(ether('1375'));
                expect(await baseVesting.getRewardBalance(ether('100'), {
                    from: user2
                })).to.be.bignumber.equal(ether('5750')); //57.5%
                expect(await baseVesting.getRewardBalance(ether('50'), {
                    from: user2
                })).to.be.bignumber.equal(ether('2875'));
            })
            it('After VestingTimeEnd', async () => {
                await time.increaseTo(startDate.add(time.duration.minutes(610)));
                expect(await baseVesting.getRewardBalance(ether('100'), {
                    from: user1
                })).to.be.bignumber.equal(ether('5000'));
                expect(await baseVesting.getRewardBalance(ether('100'), {
                    from: user2
                })).to.be.bignumber.equal(ether('10000'));
            })
        })
    });
})